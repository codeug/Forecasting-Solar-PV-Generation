...
parm = 'dhi'
new_data['converted_data'] = new_data['%s'%(parm)].fillna(0)
new_data['converted_data_'] = new_data['converted_data'][new_data['converted_data']!=0]
temp = new_data['converted_data_'].dropna()
dp.plot_graph(temp, unit, title)
##dp.plot_graph(new_data['converted_data__'], unit, title)
...
temperature_log = np.log(temp)

##dp.plot_graph(new_data['temperature_log'], unit, title)

# Removing trend variations
first_log_difference = temperature_log - temperature_log.shift()
...
decomposition = seasonal_decompose(temperature_log, model='additive', freq=30)  
fig = decomposition.plot()
plt.show()

decomposition = seasonal_decompose(first_log_difference[1:], model='additive', freq=30)  
fig = decomposition.plot()
plt.show()


ar. plot_acf_pacf(first_log_difference.dropna(inplace=False))


##model = sm.tsa.ARIMA(new_data['temperature_log'].iloc[1:], order=(2, 1, 6))
##model = sm.tsa.ARIMA(new_data['temperature_log'], order=(2, 1, 6))
model = sm.tsa.ARIMA(temperature_log, order=(4, 1, 8))
...
first_log_difference.plot(label='Original')
forecast.plot(label='model fitted')
plt.title('RSS: %.4f'% sum((results.fittedvalues.values-first_log_difference[1:].values)**2))
plt.legend(loc='best')
plt.show()
